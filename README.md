...
Внутри задач нет `psutil`, SQL-запросов, фонового профилировщика или записи
метрик. Один внешний процесс раз в секунду читает счётчики Linux `/proc`.
Время задач и очереди извлекается из metadata DB только после исполнения.
Собственная стоимость наблюдателя показывается в дашборде.

## Что здесь есть

```text
airflow-benchmark/
├── dags/resource_benchmark.py  # единственный DAG и три чистые нагрузки
├── observe.py                  # пассивный наблюдатель /proc
├── report.py                   # расчёты и автономный HTML
├── env.sh                      # изолированная конфигурация Airflow
...
Результат появится в `results/<run-id>/dashboard.html`. Файл автономный: его
можно открыть в браузере или передать вместе с `summary.json`.
...
Не сравнивайте прогоны с разными параметрами нагрузки, если цель — оценить
изменение конфигурации Airflow.

Перед каждым прогоном `run.sh` сверяет значение `parallelism` в текущем
терминале с фактическим числом LocalExecutor workers. Это предотвращает отчёты,
где подпись говорит «32», а scheduler продолжает работать с конфигурацией «16».
